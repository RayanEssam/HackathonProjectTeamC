...
//        var queryResult = false
        ...
        db.collection("Orders").whereField("email", isEqualTo: Auth.auth().currentUser!.email!).getDocuments {
            ...
            if error == nil {
                if let querySnapshotDocs = querySnapshot?.documents{
                    ...
                    for doc in querySnapshotDocs {
                        if let state = doc.get("isCompleted"){
                            if state as! Bool == false{
                                self.customAlert.showAlert(with: "خطأ :) ", message: "لديك طلب سابق غير مكتمل", on: self)
                            }
                        }
                    }
                }
